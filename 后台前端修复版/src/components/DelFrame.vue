...
export default {
  ...
  watch:{
    ...
    'delStatus':{
      handler(val){
        // console.log(val);
        if(val==1){
          this.showDeleteSuccess(this.delMsg);
        }else if(val == 2){
          this.showDeleteDefault(this.delMsg);
        }
      },
      deep:true
    }
  ...
  methods: {
    ...
    showDeleteSuccess (delMsg) {
      this.$vs.notify({
        color: 'success',
        title: '提示',
        text: delMsg ? delMsg : '删除成功'
      })
    },
    showDeleteDefault(delMsg){
      this.$vs.notify({
        color: 'danger',
        title: '提示',
        text: delMsg ? delMsg :'删除失败'
      })
    }
  },
  mounted(){
