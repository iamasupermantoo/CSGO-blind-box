...
class WebSocket1 {
    ...
    const LOG_PATH = '/application/public/';
    const LISTEN_SOCKET_NUM = 10000;
    // var $server_url = 'www.gosking.net';
       var $server_url = 'kx.qqbye.cn';
...
//                $bytes = @socket_recv($socket, $buffer, 2048, 0);
                $bytes = @socket_recv($socket, $buffer, 51200, 0);
//                echo $buffer."\n";
//                echo strlen($buffer)."\n";
//                echo "bytes:".$bytes."\n";
                //-------------push----------------
                $paramsArr = json_decode($buffer,true);
                if(isset($paramsArr['info'])){
//                    echo 'ok';
//                    $battle_id = $paramsArr['battle_id'] = 1352;
//                    $params = [
//                        'battle_id' => $battle_id
//                    ];
//                    $request_url = 'http://'.$this->server_url.'/index/battle/updateRoom';
//                    $re = $this->httpRequest($request_url, 'post',json_encode($params));
//                    $res = json_decode($re,true);//转数组
//                    $res['state'] = $paramsArr['state'];
////
//                    $msg = $this->build(json_encode($res));
//                    echo $msg."\n";
//                    $this->broadcast($msg);

                    $msg = $this->build(json_encode($paramsArr));
                    $this->broadcast($msg);
                    return;
                }
                //-------------push----------------

                $paramsArr = self::parse($buffer);
//                echo $paramsArr;
//                $id = trim($paramsArr['battle_id']);
                if(isset($paramsArr['battle_id'])){
//                    file_put_contents("log.txt","$id\n",FILE_APPEND);//记录日志
                    $battle_id = $paramsArr['battle_id'];
                    $params = [
                        'battle_id' => $battle_id
                    ];
                    $request_url = 'http://'.$this->server_url.'/index/battle/updateRoom';
...
//                        echo json_encode($recv_msg)."--------------2\n";
                    }
                }

//                array_unshift($recv_msg, 'receive_msg');
//                echo json_encode($recv_msg);
...
//                file_put_contents("log.log","$msg\n",FILE_APPEND);//记录日志
    ...
    private function dealMsg($socket, $recv_msg) {
        ...
        $msg_content = json_encode($recv_msg['content']);
        $response = [];
    ...
    public function broadcast($data) {
        foreach ($this->sockets as $socket) {
            ...
            //----------------------
//            $request_url = 'http://'.$this->server_url.'/index/battle/battleList';
//            $re = $this->httpRequest($request_url, 'post');
//            $msg = $this->build(json_encode($re));
//            $type = $data;
//            file_put_contents("log.txt","$type\n",FILE_APPEND);//记录日志
//            socket_write($socket['resource'], $msg, strlen($msg));
            //----------------------
//            $data[] = $data;
//            $data[] = json_encode($socket['resource']);
...
$ws = new WebSocket1("0.0.0.0", "8665");
//$ws = new WebSocket1("192.168.31.72", "7654");
//$ws = new WebSocket1("0.0.0.0", "7654");